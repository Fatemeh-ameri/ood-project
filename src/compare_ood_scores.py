...
# Energy scores
energy_t1_scores = torch.logsumexp(all_logits, dim=1)

temperature = 2
energy_t2_scores = temperature * torch.logsumexp(
    all_logits / temperature,
    dim=1
)

# Max logit score
max_logit_scores = torch.max(all_logits, dim=1).values

# Logit margin score
top2_logits = torch.topk(all_logits, k=2, dim=1).values
margin_scores = top2_logits[:, 0] - top2_logits[:, 1]

scores = {
    "MSP": msp_scores.tolist(),
    "Energy T=1": energy_t1_scores.tolist(),
    "Energy T=2": energy_t2_scores.tolist(),
    "Max Logit": max_logit_scores.tolist(),
    "Logit Margin": margin_scores.tolist(),
}

print()
print("OOD Score Comparison")
print("--------------------")

for score_name, score_values in scores.items():
    auroc = roc_auc_score(all_labels, score_values)
    print(f"{score_name:15s} AUROC: {auroc:.4f}")